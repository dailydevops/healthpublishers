...
/// <summary>
/// A minimal, real HTTP listener standing in for an arbitrary user-configured webhook endpoint. Since a generic
/// webhook has no specific external product to target, there is no Testcontainers module to host - the "server"
/// is whatever the user points it at. Accepts every request with <see cref="HttpStatusCode.OK"/> so publishers can
/// perform a real network round-trip; per-request assertions are made through a
/// <see cref="Internals.CapturingHttpMessageHandler"/> attached to the calling publisher's <see cref="HttpClient"/>.
/// </summary>
public sealed class WebhookMockServer : IAsyncInitializer, IAsyncDisposable
{
    ...
    public Task InitializeAsync()
    {
        ...
        ServerUrl = new Uri($"http://127.0.0.1:{port}/webhooks/health");
