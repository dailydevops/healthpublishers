...
/// <summary>
/// A minimal, real HTTP listener standing in for the Opsgenie Alert API, since no Testcontainers-hosted or
/// publicly available emulator exists for Opsgenie's SaaS-only alerting service. Accepts every create-alert
/// request with <see cref="HttpStatusCode.Accepted"/> and every close-alert request with
/// <see cref="HttpStatusCode.Accepted"/> as well, so publishers can perform a real network round-trip;
/// per-request assertions are made through a <see cref="Internals.CapturingHttpMessageHandler"/> attached to
/// the calling publisher's <see cref="HttpClient"/>.
/// </summary>
public sealed class OpsgenieMockServer : IAsyncInitializer, IAsyncDisposable
{
