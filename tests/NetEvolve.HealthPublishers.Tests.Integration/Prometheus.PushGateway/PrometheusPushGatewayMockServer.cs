...
/// <summary>
/// A minimal, real HTTP listener standing in for a Prometheus Pushgateway instance, since no
/// Testcontainers-hosted equivalent exists. Accepts every request with <see cref="HttpStatusCode.Accepted"/>
/// so publishers can perform a real network round-trip; per-request assertions are made through a
/// <see cref="Internals.CapturingHttpMessageHandler"/> attached to the calling publisher's <see cref="HttpClient"/>.
/// </summary>
public sealed class PrometheusPushGatewayMockServer : IAsyncInitializer, IAsyncDisposable
{
