...
/// <summary>
/// A minimal, real HTTP listener standing in for a Microsoft Teams incoming webhook, since Microsoft Teams
/// webhooks/workflow connectors are a pure external SaaS HTTP endpoint with no Testcontainers-hosted or
/// otherwise publicly available emulator. Accepts every request with <see cref="HttpStatusCode.OK"/> so
/// publishers can perform a real network round-trip; per-request assertions are made through a
/// <see cref="Internals.CapturingHttpMessageHandler"/> attached to the calling publisher's <see cref="HttpClient"/>.
/// </summary>
public sealed class MicrosoftTeamsMockServer : IAsyncInitializer, IAsyncDisposable
{
    ...
    public Uri WebhookUrl => new(ServerUrl, "webhookb2/00000000-0000-0000-0000-000000000000/IncomingWebhook/token");
