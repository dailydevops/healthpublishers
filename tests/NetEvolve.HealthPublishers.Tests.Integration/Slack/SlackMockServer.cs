...
/// <summary>
/// A minimal, real HTTP listener standing in for a Slack incoming webhook endpoint, since no
/// Testcontainers-hosted or public emulator equivalent exists for Slack. Accepts every request with
/// <see cref="HttpStatusCode.OK"/> so publishers can perform a real network round-trip; per-request
/// assertions are made through a <see cref="Internals.CapturingHttpMessageHandler"/> attached to the
/// calling publisher's <see cref="HttpClient"/>.
/// </summary>
public sealed class SlackMockServer : IAsyncInitializer, IAsyncDisposable
{
    ...
    public Task InitializeAsync()
    {
        ...
        ServerUrl = new Uri($"http://127.0.0.1:{port}/services/T000/B000/XXX");
