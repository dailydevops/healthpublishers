...
public sealed class OpsgenieOptionsConfigureTests
{
    [Test]
    public void Configure_WhenArgumentNameWhitespace_ThrowArgumentException()
    {
        // Arrange
        var configure = new OpsgenieOptionsConfigure(new ConfigurationBuilder().Build());
        var options = new OpsgenieOptions();
    ...
    public async Task Configure_WhenArgumentNameNull_UsesDefaultNameSection()
    {
        ...
        {
            { "HealthPublishers:Opsgenie:Default:ApiKey", "test-key" },
        };
        var configuration = new ConfigurationBuilder().AddInMemoryCollection(values).Build();
        var configure = new OpsgenieOptionsConfigure(configuration);
        var options = new OpsgenieOptions();
    ...
    public async Task Configure_WhenCalledWithoutName_UsesDefaultNameSection()
    {
        ...
        // Act
        ((IConfigureOptions<OpsgenieOptions>)configure).Configure(options);
    ...
    public async Task Validate_WhenNameWhitespace_ReturnFailure()
    {
        // Arrange
        var configure = new OpsgenieOptionsConfigure(new ConfigurationBuilder().Build());
        var options = new OpsgenieOptions { ApiKey = "test-key", SystemIdentifier = "checkout-service" };
    ...
    public async Task Validate_WhenApiKeyNullOrWhiteSpace_ReturnFailure(string? apiKey)
    {
        // Arrange
        var configure = new OpsgenieOptionsConfigure(new ConfigurationBuilder().Build());
        var options = new OpsgenieOptions { ApiKey = apiKey!, SystemIdentifier = "checkout-service" };
    ...
    public async Task Validate_WhenSystemIdentifierNullOrWhiteSpace_ReturnFailure(string? systemIdentifier)
    {
        // Arrange
        var configure = new OpsgenieOptionsConfigure(new ConfigurationBuilder().Build());
        var options = new OpsgenieOptions { ApiKey = "test-key", SystemIdentifier = systemIdentifier! };
    ...
    public async Task Configure_WhenConfigurationAvailable_ExpectedValues()
    {
        ...
        var values = new Dictionary<string, string?>(StringComparer.Ordinal)
        {
            { "HealthPublishers:Opsgenie:Test:ApiUrl", "https://api.eu.opsgenie.com" },
            { "HealthPublishers:Opsgenie:Test:ApiKey", "test-key" },
            { "HealthPublishers:Opsgenie:Test:SystemIdentifier", "checkout-service" },
        };
        var configuration = new ConfigurationBuilder().AddInMemoryCollection(values).Build();
        var configure = new OpsgenieOptionsConfigure(configuration);
        var options = new OpsgenieOptions();
        ...
        using (Assert.Multiple())
        {
            _ = await Assert.That(options.ApiUrl).IsEqualTo(new Uri("https://api.eu.opsgenie.com"));
            _ = await Assert.That(options.ApiKey).IsEqualTo("test-key");
