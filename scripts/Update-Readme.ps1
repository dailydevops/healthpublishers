<#
.SYNOPSIS
    Updates README files with the latest NuGet package information from the NetEvolve.HealthPublishers repository.

.DESCRIPTION
    This script fetches all published NetEvolve.HealthPublishers packages from NuGet.org and updates
...
.EXAMPLE
    .\Update-Readme.ps1 -WorkingDirectory "C:\src\healthpublishers"
...
<#
.SYNOPSIS
    Retrieves all NetEvolve.HealthPublishers packages from NuGet.org and formats them as a Markdown table.

.DESCRIPTION
    Queries the NuGet API to fetch all packages matching "netevolve.healthpublishers", including prerelease
    versions. The function handles pagination (100 packages per request) and filters results to only
    include packages from the dailydevops/healthpublishers repository. Returns a formatted Markdown table
...
function Get-Packages {
  ...
  $repositoryUrl = 'https://github.com/dailydevops/healthpublishers'

  # Paginate through all available packages
  do {
    # Build query URL with pagination parameters and prerelease inclusion
    $queryUrl = "https://azuresearch-usnc.nuget.org/query?q=netevolve.healthpublishers&take=$($take)&skip=$($skip)&prerelease=true&semVerLevel=2.0.0"
